...
function loadData() {
    ...
    if (stored) {
        ...
        // Merge settings but ignore old theme/sound keys if present
        if (parsed.settings) {
            state.settings.intervalMinutes = parsed.settings.intervalMinutes || 150;
            state.settings.defaultFormulaType = parsed.settings.defaultFormulaType || '';
            state.settings.defaultBreastfeedingSide = parsed.settings.defaultBreastfeedingSide || 'Both';
        }
    }
    // Sync settings UI
...
function renderEvents() {
    ...
    if (state.events.length === 0) {
        html = `<li class="empty-state">No activity recorded yet.<br>Start a feed or log a diaper!</li>`;
    } else {
        html = state.events.map(event => {
